...
function validateAuthOptions(authMethods: OptAuthMethod[], errors: string[]): void {
  ...
  const isCfMtls = authMethods.includes(OptAuthMethod.CfMtls);
  const isKymaMtls = authMethods.includes(OptAuthMethod.KymaMtls);

  if (isOpen && (isBasicAuth || isCfMtls || isKymaMtls)) {
    errors.push('Authentication method "open" cannot be used together with other options.');
    return;
  }

  if (isCfMtls && isKymaMtls) {
    errors.push('Authentication method "cf-mtls" cannot be used together with method "kyma-mtls".');
    return;
  }
  ...
  if (isKymaMtls) {
    const trustedCerts = process.env.KYMA_MTLS_TRUSTED_CERTS;

    if (!trustedCerts || trustedCerts.trim() === "") {
      errors.push("SAP BTP (Kyma runtime) authentication requires KYMA_MTLS_TRUSTED_CERTS to be configured.");
      return;
    }

    // Validate KYMA_MTLS_TRUSTED_CERTS JSON structure
    try {
      ...
      if (!parsed || typeof parsed !== "object") {
        errors.push("KYMA_MTLS_TRUSTED_CERTS must be a JSON object");
        return;
      }
      ...
      if (parsed.certs !== undefined && !Array.isArray(parsed.certs)) {
        errors.push("KYMA_MTLS_TRUSTED_CERTS.certs must be an array if provided");
        return;
      }

      if (parsed.certs === undefined && !hasConfigEndpoints) {
        errors.push("KYMA_MTLS_TRUSTED_CERTS.certs is required when no configEndpoints are provided");
        return;
      }
      ...
      if (parsed.certs && parsed.certs.length === 0) {
        errors.push(
          'KYMA_MTLS_TRUSTED_CERTS.certs cannot be empty. Use explicit wildcards like {"issuer": "*", "subject": "*"} or omit certs when using configEndpoints',
        );
        return;
      }
      ...
      if (parsed.certs) {
        for (const cert of parsed.certs) {
          if (!cert.issuer || typeof cert.issuer !== "string") {
            errors.push("Each cert entry in KYMA_MTLS_TRUSTED_CERTS must have a valid 'issuer' string");
            return;
          }
          if (!cert.subject || typeof cert.subject !== "string") {
            errors.push("Each cert entry in KYMA_MTLS_TRUSTED_CERTS must have a valid 'subject' string");
            return;
          }
        }
      }
      ...
      if (parsed.configEndpoints !== undefined) {
        if (!Array.isArray(parsed.configEndpoints)) {
          errors.push("KYMA_MTLS_TRUSTED_CERTS.configEndpoints must be an array");
          return;
        }
        for (const endpoint of parsed.configEndpoints) {
          if (typeof endpoint !== "string" || endpoint.trim() === "") {
            errors.push("Each configEndpoint in KYMA_MTLS_TRUSTED_CERTS must be a non-empty string");
            return;
          }
        }
      }
    } catch (error) {
      errors.push(`Invalid JSON in KYMA_MTLS_TRUSTED_CERTS: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
